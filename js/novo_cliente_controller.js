/**
 * CONTROLLER - Novo Cadastro Happy Pet (Versão Final e Estabilizada)
 * Nenhuma lógica de reforço, CPF ou máscaras foi removida.
 */
...
// --- BUSCA DE CEP AUTOMÁTICA (Corrigida: Anti-Loop e Sem Erro de Variável) ---
function configurarCEP() {
    ...
    if(inputCep) {
        inputCep.addEventListener('blur', (e) => {
            ...
            if(cep.length === 8 && !buscandoAgora) {
                buscandoAgora = true;
                const inputEnd = document.getElementById('cliEndereco');
                
                const valorAnterior = inputEnd.value;
                inputEnd.value = "Localizando endereço...";
                ...
                Utils.consultarCEP(cep, (dados) => {
                    if(dados && !dados.erro) {
                        ...
                        if(cep !== "") alert("CEP não encontrado.");
                        inputEnd.value = valorAnterior;
                    }
                    setTimeout(() => { buscandoAgora = false; }, 500);
                });
            }
        });
...
// --- ETAPA 1: LÓGICA DO TUTOR (Com Validação de CPF e Busca por Existente) ---
function configurarEventosTutor() {
    ...
    if(cpfInput) {
        cpfInput.addEventListener('blur', async (e) => {
            ...
            // Validação de formato/algoritmo real do Utils
...
// --- ETAPA 2: LÓGICA DO PET & VACINAS (Com Lógica de 21 dias para filhotes) ---
...
function configurarEventosPet() {
    ...
    const calcularRetorno = () => {
        ...
        if (selected && dataDose && selected.value !== "") {
            ...
            // LÓGICA DE FILHOTE: Se menos de 4 meses, reforço em 21 dias
            if (dataNasc) {
