/**
 * CONTROLLER - Novo Cadastro Happy Pet (Versão Definitiva e à Prova de Falhas)
 */
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    configurarCEPIndependente();
    configurarEventosTutor();
    configurarEventosPet();
});

// --- BUSCA DE CEP AUTOMÁTICA (Independente do Utils.js para evitar falhas) ---
function configurarCEPIndependente() {
...
function configurarEventosTutor() {
    ...
    if(cpfInput) {
        cpfInput.addEventListener('blur', async (e) => {
            ...
            // Trava de segurança caso o utils.js demore a carregar
            if (cpfLimpo.length > 0 && typeof Utils !== 'undefined' && Utils.validarCPF && !Utils.validarCPF(cpfLimpo)) {
                alert("⚠️ CPF Inválido! Verifique os números.");
    ...
    document.getElementById('formTutor').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            alert("✅ Tutor salvo com sucesso! Cadastre o Pet.");
...
function configurarEventosPet() {
    ...
    const btnMostrar = document.getElementById('btnMostrarFormPet');
    ...
    selEspecie.onchange = (e) => {
        ...
        // CORRIGIDO: Tag option agora está devidamente fechada!
    ...
    const calcRetorno = () => {
        const sel = selVacina.options[selVacina.selectedIndex];
        const dDose = document.getElementById('dataVac').value;
        const dNasc = document.getElementById('petNascimento').value;
        
        if(sel && dDose && sel.value !== "") {
            let dias = parseInt(sel.dataset.reforco) || 365;
            if (dNasc) {
                const ms = new Date(dDose+'T12:00:00') - new Date(dNasc+'T12:00:00');
                if (ms / (1000*60*60*24*30) < 4 && ['V10','V8','V5'].includes(sel.value)) dias = 21;
            }
            let [ano, mes, dia] = dDose.split('-');
            let dt = new Date(ano, mes - 1, dia);
            dt.setDate(dt.getDate() + dias);
            document.getElementById('dataRevacinaPrevista').value = dt.toISOString().split('T')[0];
        }
    };

    selVacina.onchange = calcRetorno;
    document.getElementById('dataVac').onchange = calcRetorno;

    document.getElementById('btnAddVacinaLista').onclick = () => {
        const nome = selVacina.value;
        const dose = document.getElementById('dataVac').value;
        const revac = document.getElementById('dataRevacinaPrevista').value;
        
        if(!nome || !dose || !revac) return alert("Preencha a vacina e datas.");
        
        listaVacinasTemp.push({ nome, data_aplicacao: dose, data_revacina: revac });
        
        document.getElementById('miniListaVacinas').innerHTML += `<div style="background:#eee; padding:8px; margin-top:5px; border-radius:5px; font-size:0.8rem; border-left: 3px solid var(--primary);"><b>${nome}</b><br><small>Reforço: ${revac.split('-').reverse().join('/')}</small></div>`;
        
        selVacina.value = ""; document.getElementById('dataVac').value = ""; document.getElementById('dataRevacinaPrevista').value = "";
    };

    formPet.onsubmit = async (e) => {
        e.preventDefault();
        if(!tutorIdAtual) return alert("Salve o tutor primeiro.");
        
        const btn = e.target.querySelector('button[type="submit"]');
        const comp = document.querySelector('input[name="comportamento"]:checked');
        
        let proximaRevacina = '2099-12-31';
        if(listaVacinasTemp.length > 0) {
            let dts = listaVacinasTemp.map(v => new Date(v.data_revacina+'T12:00:00').getTime());
            proximaRevacina = new Date(Math.min(...dts)).toISOString().split('T')[0];
        }
